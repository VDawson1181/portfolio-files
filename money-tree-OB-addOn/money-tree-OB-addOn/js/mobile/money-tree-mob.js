/* Money Tree Device - balance v1.1 */
	...
	if(!$("body.eo").length){//not EO? read data, change display, bind clicks as needed
		...
		if (addOnDevice.isActionTaken) {
			if(!savingsCookie){
				...
				$(deviceIdentifier+" .mt-sign-element, "+deviceIdentifier+" .mt-dir-msg").css({"display":"none"});
				$(deviceIdentifier+" .mt-result-element").css({"display":"block"});
				
				/*$(deviceIdentifier).fadeIn("slow").css({"cursor":"pointer"}).click(function(){
					startSaveTree(deviceIdentifier);
				});*/
				
				$(deviceIdentifier).on("tap", function(e){ 
					startSaveTree(deviceIdentifier);
				});
				...
				$(deviceIdentifier+" .mt-sign-element, "+deviceIdentifier+" .playArea li.mtMoney, "+deviceIdentifier+" .mt-tree-element, "+deviceIdentifier+" ul.playArea").css({"display":"none"});				
			...
			$(deviceIdentifier).on("tap", function(e){ 
				...
				$(deviceIdentifier+" .mt-sign-element").fadeOut("fast", function(){
			...
			//Money Tree click --TO be deleted
			/*$(deviceIdentifier).css({"cursor":"pointer"}).click(function(){	
				$(this).css({"cursor":"default"}).unbind( "click" );
				$(deviceIdentifier+" .mt-sign-element").fadeOut("fast", function(){
					//$(deviceIdentifier+" .mt-sign-element").removeClass("raiseTheSign");
					$(deviceIdentifier+", "+deviceIdentifier+" .mt-tree-element").removeClass("mt-wobble");
...
function deviceAnimate(deviceIdentifier){
	...
	for(;moneyAmt<=20;moneyAmt++){
		...
		setTimeout(function(){			
			...
			//setTimeout(function(){			
				$(deviceIdentifier+" .mt-sign-element").fadeOut("fast",function(){										
					$(deviceIdentifier+" .mt-result-element").fadeIn("slow", function(){
						/*$(deviceIdentifier).css({"cursor":"pointer"}).click(function(){		
							$(this).css({"cursor":"default"}).unbind( "click" );					
							startSaveTree(deviceIdentifier);
						});*/	
...
function startSaveTree(deviceIdentifier){
		...
		//Savings Tree tap	
		$(deviceIdentifier+" .mt-sav-tree-element, "+deviceIdentifier+" .mt-sav-sign-element").on("tap", function(e){ 
			$(this).css({"cursor":"default"}).unbind( "click" );
		...
		//Savings Tree click --TO be deleted			
		//$(deviceIdentifier+" .mt-sav-tree-element, "+deviceIdentifier+" .mt-sav-sign-element, "+deviceIdentifier+".mt-hdr-savings-name").css({"cursor":"pointer"}).click(function(){	
		/*$(deviceIdentifier).css({"cursor":"pointer"}).click(function(){	
			$(this).css({"cursor":"default"}).unbind( "click" );
			$(deviceIdentifier+" .mt-sav-sign-element").removeClass("raiseTheSign");
			$(deviceIdentifier+", "+deviceIdentifier+" .mt-sav-tree-element").removeClass("mt-wobble");
...
function deviceFinished(deviceId){	
	...
	if($(".spectrumLightbox_content2 "+deviceId).length){
		$(deviceId+" .pkg_closeButton").css({"display":"block"});						
	};
	
	isDeviceInLightbox(deviceId);
